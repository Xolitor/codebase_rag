...
def run_hybrid_search_benchmark():
    project_root = Path(__file__).resolve().parent
    repo_path = project_root / "repo"
    output_path = project_root / "hybrid_search_results.json"

    if not repo_path.exists() or not repo_path.is_dir():
        raise FileNotFoundError(f"Expected repo folder at: {repo_path}")

    print("[benchmark] re-initializing vector collection and ingesting repo/")
    init_collection()
    ...
    for index, query in enumerate(QUERIES, start=1):
        print(f"[benchmark] ({index}/{len(QUERIES)}) query: {query}")
            ...
            chunk_rows.append(
                {
                    ...
                    "vector_similarity": chunk.get("vector_score"),
